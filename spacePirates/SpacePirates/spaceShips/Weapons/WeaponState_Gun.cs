...
{
    class WeaponState_LolCannon : IWeapon
    {
        String name;
        double time;
        double fireRate;

        public WeaponState_LolCannon()
        {
            name = "Lolcannon";
            time = 0;
            fireRate = 20;
        ...
        public void Fire(GameTime gameTime, Unit ship)
        {
            ...
            {
                int speed = 300;
                float rotatio = (float)(new Random()).NextDouble() * MathHelper.TwoPi;
                Vector2 vel = new Vector2((float)(Math.Sin(rotatio) * speed), (float)(Math.Cos(rotatio) * speed));
